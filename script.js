...
function checkout() {
  ...
  showPaymentModal({ orderNum, tgl, jam, totalPrice, itemsHTML, itemsText });
}
...
function showPaymentModal(orderData) {
  ...
  overlay.innerHTML = `
    <div class="receipt-box payment-modal">
      ...
      <div class="receipt-summary">
        <div class="receipt-row"><span>Subtotal</span><span>${formatPrice(orderData.totalPrice)}</span></div>
  ...
  if (confirmBtn) {
    confirmBtn.addEventListener("click", () => {
      ...
      const waMessage = encodeURIComponent(`Halo Lifa Flora! 🌸\n\nSaya ingin memesan buket berikut:\n${orderData.itemsText}\n\nNo. Pesanan: #${orderData.orderNum}\nTanggal: ${orderData.tgl} ${orderData.jam} WIB\nTotal: ${formatPrice(orderData.totalPrice)}\nPembayaran: Transfer SeaBank 901809290073\nNama: ${customerName}\nNomor Telepon: ${customerPhone}\nAlamat Lengkap: ${customerAddress}\n\nSilakan lanjutkan pesanan Anda setelah mengirimkan bukti pembayaran. Untuk pembayaran non-COD, wajib menyertakan transaksi atau bukti pembayaran beserta alamat lengkap untuk pengiriman, dan pesanan tidak bisa diproses tanpa bukti pembayaran serta alamat lengkap.\n\n*GRATIS ONGKIR UNTUK DAERAH BANDUNG.*\n\nTerima kasih telah memesan di toko kami 💗🌷\nSemoga buket pilihan Anda semakin menawan 💐`);
